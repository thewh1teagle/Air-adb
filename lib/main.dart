...
Future<void> startAdbServer() async {
  var result = await Process.run(
    'su',
    ['-c', 'setprop', 'service.adb.tcp.port', '5555'],
  );
...
Future<void> stopAdbServer() async {
  var result = await Process.run(
    'su',
    ['-c', 'setprop', 'service.adb.tcp.port', '-1'],
  );
  result = await Process.run(
    'su',
    ['-c', 'stop', 'adbd'],
...
class MyApp extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return MaterialApp(
      ...
      home: MyHomePage(title: 'Adb server'),
    );
  }
}
...
class _MyHomePageState extends State<MyHomePage> {
  int _counter = 0;
  final Future<String?> _localIp = getLocalIpAddress();
    ...
    return Scaffold(
      ...
      body: Center(
        ...
        child: Column(
          ...
          children: <Widget>[
            TextButton(
              onPressed: startAdbServer,
              child: Text("start adb server"),
              style: ButtonStyle(
                foregroundColor: MaterialStateProperty.all<Color>(Colors.blue),
              ),
            ),
            TextButton(
              onPressed: stopAdbServer,
              child: Text("stop adb server"),
              style: ButtonStyle(
                foregroundColor: MaterialStateProperty.all<Color>(Colors.blue),
              ),
            ),
            FutureBuilder(
              ...
              builder: (BuildContext context, AsyncSnapshot snapshot) {
                ...
                final String localIp = snapshot.data;
                return Text(
